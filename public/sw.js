const CACHE_NAME = 'yupos-shell-v10';
...
self.addEventListener('fetch',(event)=>{
  ...
  const isNavigation=url.pathname.endsWith('/index.html')||event.request.mode==='navigate'; const isManifest=url.pathname.endsWith('/manifest.webmanifest'); const isServiceWorker=url.pathname.endsWith('/sw.js'); const isAppIcon=/\/assets\/(icon-192|icon-512|yupos-app-icon)\.png$/.test(url.pathname);
  if(isNavigation||isManifest||isServiceWorker||isAppIcon){
