...
const syncPatchedOrder = async (uid: string, businessType: string, patchedOrder: any) => {
  ...
  if (index >= 0) {
    remote[index] = { ...remote[index], ...patchedOrder, merchantId: uid, businessType };
  } else {
    remote.push({ ...patchedOrder, merchantId: uid, businessType });
  }

  await setDoc(
    ref,
    { list: remote, merchantId: uid, businessType, updatedAt: Date.now() },
    { merge: true },
  );
};
...
const renderManager = (force = false) => {
  ...
  container.innerHTML = `
    ...
    <div class="yp-pay-modal" style="display:none">
      <div class="yp-pay-card">
        <div class="yp-pay-head">
          <div><div class="yp-pay-title">Metode Pembayaran Custom</div><div class="yp-pay-sub">Tambahkan metode seperti Ojek Online, Delivery, COD, atau lainnya.</div></div>
          <button class="yp-pay-close" type="button">×</button>
        </div>
        ...
        <div class="yp-pay-presets">
          <button class="yp-pay-preset" type="button" data-method="Ojek Online">+ Ojek Online</button>
          <button class="yp-pay-preset" type="button" data-method="Delivery">+ Delivery</button>
          <button class="yp-pay-preset" type="button" data-method="COD">+ COD</button>
          <button class="yp-pay-preset" type="button" data-method="Kasbon">+ Kasbon</button>
        </div>
        <div class="yp-pay-list">
          ${methods.length ? methods.map((method) => `<div class="yp-pay-item"><span class="yp-pay-name">${method.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</span><button class="yp-pay-delete" type="button" data-delete="${encodeURIComponent(method)}">Hapus</button></div>`).join('') : '<div style="font-size:11px;color:#94a3b8;padding:8px 0">Belum ada metode custom.</div>'}
        </div>
      </div>
    </div>
  `;
  ...
  const addMethod = async (value: string) => {
    ...
    if (!method) return;
    if (loadMethods(session.uid).includes(method)) return;
  ...
  container.querySelectorAll<HTMLElement>('[data-method]').forEach((button) => {
    button.addEventListener('click', () => void addMethod(button.dataset.method || ''));
  });
  container.querySelectorAll<HTMLElement>('[data-delete]').forEach((button) => {
...
const triggerCustomPayment = (method: string) => {
  const session = getSession();
  if (!session) return;
  const cashButton = Array.from(document.querySelectorAll<HTMLButtonElement>('button')).find(
    (button) => button.textContent?.trim() === 'CASH (TUNAI)',
  );
...
const injectPaymentButtons = () => {
  ...
  wrapper.querySelectorAll<HTMLButtonElement>('[data-yupos-custom-payment]').forEach((button) => {
    button.addEventListener('click', () => triggerCustomPayment(decodeURIComponent(button.dataset.yuposCustomPayment || '')));
  });
};
...
const renderRevenueCustomBreakdown = () => {
  ...
  const html = `
    <div class="yp-rev-title">💳 Omzet Metode Custom</div>
    <div class="yp-rev-sub">Hari ini · tersimpan berdasarkan metode pembayaran</div>
    ${totals.map((item) => `<div class="yp-rev-line"><span>${item.method.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</span><span class="yp-rev-value">${formatRp(item.total)}</span></div>`).join('')}
  `;
