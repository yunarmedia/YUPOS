...
export async function syncConfigToFirebase(settings: StoreSettings, merchantId: string = ''): Promise<boolean> { try { const id = requireMerchantId(merchantId); await setDoc(doc(db, 'yupos_config', id, 'settings', 'data'), { ...settings, merchantId: id }, { merge: true }); saveMerchantSettings(id, settings, true); return true; } catch (err) { console.warn('Firebase config sync warning:', err); return false; } }
export async function syncProductsToFirebase(products: ProductItem[], merchantId: string = '', businessType: BusinessType = 'barbershop'): Promise<boolean> { try { const id = requireMerchantId(merchantId); await setDoc(doc(db, 'yupos_catalog', id, businessType, 'products'), { items: products.map(p => ({ ...p, merchantId: id, businessType })), merchantId: id, businessType, updatedAt: Date.now() }, { merge: true }); saveMerchantProducts(id, businessType, products, true); return true; } catch (err) { console.warn('Firebase products sync warning:', err); return false; } }
export async function syncOrdersToFirebase(orders: Order[], merchantId: string = '', businessType: BusinessType = 'barbershop'): Promise<boolean> { try { const id = requireMerchantId(merchantId); const normalized = normalizeOrderIds(dedupeOrders(orders.map(o => ({ ...o, merchantId: id, businessType })))); await setDoc(doc(db, 'yupos_transactions', id, businessType, 'orders'), { list: normalized, merchantId: id, businessType, updatedAt: Date.now() }, { merge: true }); saveMerchantOrders(id, businessType, normalized, true); return true; } catch (err) { console.warn('Firebase orders sync warning:', err); return false; } }
export async function syncExpensesToFirebase(expenses: Expense[], merchantId: string = '', businessType: BusinessType = 'barbershop'): Promise<boolean> { try { const id = requireMerchantId(merchantId); await setDoc(doc(db, 'yupos_finances', id, businessType, 'expenses'), { list: expenses.map(e => ({ ...e, merchantId: id, businessType })), merchantId: id, businessType, updatedAt: Date.now() }, { merge: true }); saveMerchantExpenses(id, businessType, expenses, true); return true; } catch (err) { console.warn('Firebase expenses sync warning:', err); return false; } }
export async function syncPettyCashToFirebase(amount: number, merchantId: string = '', businessType: BusinessType = 'barbershop'): Promise<boolean> { try { const id = requireMerchantId(merchantId); await setDoc(doc(db, 'yupos_finances', id, businessType, 'pettyCash'), { amount: Number(amount) || 0, merchantId: id, businessType, updatedAt: Date.now() }, { merge: true }); saveMerchantPettyCash(id, businessType, amount, true); return true; } catch (err) { console.warn('Firebase petty cash sync warning:', err); return false; } }
