...
export function buildMembershipSnapshot(customer: Customer): MembershipSnapshot {
  return {
    app: 'YUPOS', version: 1, code: customer.customerCode, name: normalizeText(customer.name), phone: customer.phone,
    visits: customer.visitCount || 0, memberSince: customer.memberSince, totalSpent: customer.totalSpent || 0,
    lastVisit: customer.lastVisit || '-', reward: getMembershipReward(customer), visitDetails: (customer.membershipVisits || []).slice(-10),
  };
}

export function buildMembershipScanUrl(customer: Customer): string {
  const payload = toBase64Url(JSON.stringify(buildMembershipSnapshot(customer)));
