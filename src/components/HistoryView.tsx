...
export const HistoryView: React.FC<HistoryViewProps> = ({ orders, settings, onEditOrder, onReprintOrder, onCancelOrder, onDeleteOrderPermanently, onShowToast }) => {
  ...
  const shareReceiptWhatsApp = async (order: Order) => {
    ...
    try {
      const result = await shareReceiptImage(order, settings);
      if (result === 'shared') onShowToast('Gambar struk siap dibagikan. Pilih WhatsApp lalu pilih customer tujuan.', 'success');
      else if (result === 'fallback') onShowToast('WhatsApp dibuka dengan pesan bukti transaksi. Browser ini belum mendukung pengiriman gambar langsung.', 'info');
    } catch (error) {
      if ((error as DOMException)?.name !== 'AbortError') onShowToast('Gagal membuat atau membagikan gambar struk.', 'error');
    } finally { setSharingOrderId(null); }
  };

  return <div className="flex h-full min-h-0 flex-col gap-4 p-3 sm:p-5">
    <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2 text-xl font-black text-slate-900"><HistoryIcon className="text-blue-600" /> Riwayat Transaksi</div><p className="text-sm text-slate-500">Kelola, cetak ulang, bagikan bukti struk + gambar, dan otorisasi transaksi.</p></div><div className="relative w-full sm:w-80"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18}/><input value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder="Cari transaksi..." className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-10 pr-3 outline-none focus:border-blue-500"/></div></header>
    <div className="flex gap-2 overflow-x-auto pb-1">{(['selesai','pending','batal','all'] as const).map(f=><button key={f} onClick={()=>setActiveFilter(f)} className={`whitespace-nowrap rounded-xl px-4 py-2 text-sm font-bold ${activeFilter===f?'bg-blue-600 text-white':'bg-white text-slate-600 border border-slate-200'}`}>{f==='all'?'Semua':f==='selesai'?'Selesai':f==='pending'?'Pending':'Dibatalkan'}</button>)}</div>
    <div className="min-h-0 flex-1 overflow-auto rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="min-w-[980px]"><div className="grid grid-cols-[1.05fr_1fr_1fr_.75fr_.8fr_auto] gap-3 border-b bg-slate-50 px-4 py-3 text-xs font-black uppercase text-slate-500"><span>Transaksi</span><span>Pelanggan</span><span>Waktu</span><span>Status</span><span>Total</span><span>Aksi</span></div>{filteredOrders.map(order=><div key={order.id} className="grid grid-cols-[1.05fr_1fr_1fr_.75fr_.8fr_auto] items-center gap-3 border-b px-4 py-3 text-sm last:border-0"><div><div className="font-bold">#{order.id}</div><div className="text-[10px] text-slate-400">{order.payment}</div></div><div><div>{order.customer || 'Umum'}</div><div className="text-[10px] text-slate-400">{order.customerPhone || '-'}</div></div><div>{order.date} {order.time}</div><div><span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-bold">{order.status}</span></div><div className="font-black">{formatRp(order.total)}</div><div className="flex gap-1"><button title="Cetak" onClick={()=>onReprintOrder(order)} className="rounded-lg p-2 hover:bg-blue-50"><Printer size={16}/></button><button title="Kirim struk + gambar via WhatsApp" onClick={()=>shareReceiptWhatsApp(order)} disabled={sharingOrderId===order.id} className="rounded-lg p-2 text-emerald-600 hover:bg-emerald-50 disabled:opacity-50"><MessageCircle size={16}/></button><button title="Edit" onClick={()=>startAction('edit',order)} className="rounded-lg p-2 hover:bg-yellow-50"><Edit3 size={16}/></button><button title="Batalkan" onClick={()=>startAction('cancel',order)} className="rounded-lg p-2 hover:bg-orange-50"><XCircle size={16}/></button><button title="Hapus" onClick={()=>startAction('delete',order)} className="rounded-lg p-2 hover:bg-red-50"><Trash2 size={16}/></button></div></div>)}{filteredOrders.length===0&&<div className="p-12 text-center text-slate-500">Tidak ada transaksi.</div>}</div></div>
