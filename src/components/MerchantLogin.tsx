...
interface FieldProps { icon: React.ReactNode; children: React.ReactNode; }
interface AlertBoxProps { errorMessage: string | null; successMessage: string | null; }
...
const UNREGISTERED_MESSAGE = 'Akun Anda belum aktif. Silakan hubungi tim YUPOS untuk mendapatkan akses.';

const Field: React.FC<FieldProps> = ({ icon, children }) => (
...
const AlertBox: React.FC<AlertBoxProps> = ({ errorMessage, successMessage }) => (
  <>
    {errorMessage && (
...
export const MerchantLogin: React.FC<MerchantLoginProps> = ({ onLoginSuccess }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!normalizedEmail || !password) {
      setErrorMessage('Masukkan email dan kata sandi akun YUPOS Anda.');
      return;
    }
    setLoading(true);
    try {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto overscroll-contain bg-slate-950">
      ...
      <div className="relative z-10 flex min-h-[100dvh] items-center justify-center px-3 py-5 sm:px-6 sm:py-8">
        <div className="grid w-full max-w-6xl overflow-hidden rounded-[30px] border border-white/15 bg-white/95 shadow-[0_30px_100px_rgba(0,0,0,.5)] backdrop-blur-xl lg:grid-cols-[1fr_1fr]">
          <section className="hidden min-h-[680px] flex-col justify-between bg-gradient-to-br from-blue-700 via-blue-600 to-blue-950 p-8 text-white lg:flex xl:p-10">
            ...
            <div className="flex items-center gap-2 text-xs font-semibold text-blue-100"><ShieldCheck className="h-4 w-4 text-yellow-300" /> Akses khusus untuk pelanggan YUPOS.</div>
          </section>
          ...
          <section className="flex min-h-[620px] flex-col bg-white">
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 sm:px-8">
              ...
              <div className="hidden items-center gap-1.5 rounded-full bg-blue-50 px-3 py-1.5 text-[10px] font-black text-blue-700 sm:flex"><LockKeyhole className="h-3.5 w-3.5" /> AKSES PELANGGAN</div>
            </div>
            ...
            {currentView === 'login' && (
              <form onSubmit={handleLogin} className="flex flex-1 flex-col justify-center p-6 sm:p-8">
                <div className="mb-7"><div className="mb-2 text-xs font-black uppercase tracking-[.16em] text-yellow-600">Merchant Portal</div><h2 className="text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">Masuk ke YUPOS</h2><p className="mt-2 text-sm text-slate-500">Gunakan email dan kata sandi yang telah Anda daftarkan untuk YUPOS.</p></div>
                <div className="space-y-4">
                  <AlertBox errorMessage={errorMessage} successMessage={successMessage} />
                  <div><label className="mb-2 block text-xs font-black text-slate-700">EMAIL MERCHANT</label><Field icon={<Mail className="h-5 w-5" />}><input type="email" inputMode="email" autoCapitalize="none" autoCorrect="off" spellCheck={false} autoComplete="username" enterKeyHint="next" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="merchant@gmail.com" className="w-full rounded-2xl border border-slate-200 bg-slate-50 py-3.5 pl-12 pr-4 text-sm font-semibold outline-none transition focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100" /></Field></div>
                  <div><div className="mb-2 flex items-center justify-between"><label className="text-xs font-black text-slate-700">KATA SANDI</label><button type="button" onClick={() => switchView('forgot')} className="text-xs font-black text-blue-600 hover:text-blue-800">Lupa sandi?</button></div><Field icon={<LockKeyhole className="h-5 w-5" />}><input type={showPassword ? 'text' : 'password'} autoComplete="current-password" autoCapitalize="none" autoCorrect="off" spellCheck={false} enterKeyHint="done" required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Masukkan kata sandi" className="w-full rounded-2xl border border-slate-200 bg-slate-50 py-3.5 pl-12 pr-12 text-sm font-semibold outline-none transition focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100" /><button type="button" onClick={() => setShowPassword((v) => !v)} className="absolute right-3 rounded-xl p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700">{showPassword ? <EyeOff className="h-5 w-5" /> : <Eye className="h-5 w-5" />}</button></Field></div>
                  <button type="submit" disabled={loading} className="mt-2 flex w-full items-center justify-center gap-2 rounded-2xl bg-blue-600 px-5 py-4 text-sm font-black text-white shadow-lg shadow-blue-600/25 transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60">{loading ? 'Memproses...' : <>Buka Aplikasi Kasir <ArrowRight className="h-5 w-5" /></>}</button>
                </div>
                <div className="mt-7 rounded-2xl border border-yellow-200 bg-yellow-50 p-4"><div className="flex gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-yellow-600" /><div><div className="text-xs font-black text-slate-900">Akses khusus YUPOS</div><div className="mt-1 text-[11px] leading-5 text-slate-600">Halaman ini diperuntukkan bagi pelanggan YUPOS yang telah memiliki akses.</div></div></div></div>
              </form>
            )}
            ...
            {currentView === 'forgot' && (
              <form onSubmit={handleResetPassword} className="flex flex-1 flex-col justify-center p-6 sm:p-8">
                ...
                <div className="mb-7"><div className="mb-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-50 text-blue-600"><KeyRound className="h-6 w-6" /></div><h2 className="text-2xl font-black tracking-tight text-slate-950">Pulihkan kata sandi</h2><p className="mt-2 text-sm leading-6 text-slate-500">Masukkan email yang digunakan untuk akun YUPOS Anda. Jika tersedia, tautan pemulihan akan dikirim ke email tersebut.</p></div>
                <div className="space-y-4"><AlertBox errorMessage={errorMessage} successMessage={successMessage} /><div><label className="mb-2 block text-xs font-black text-slate-700">EMAIL TERDAFTAR</label><Field icon={<Mail className="h-5 w-5" />}><input type="email" inputMode="email" autoCapitalize="none" autoCorrect="off" spellCheck={false} autoComplete="email" enterKeyHint="done" required value={resetEmail} onChange={(e) => setResetEmail(e.target.value)} placeholder="merchant@gmail.com" className="w-full rounded-2xl border border-slate-200 bg-slate-50 py-3.5 pl-12 pr-4 text-sm font-semibold outline-none transition focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100" /></Field></div><button type="submit" disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-blue-600 px-5 py-4 text-sm font-black text-white shadow-lg shadow-blue-600/25 transition hover:bg-blue-700 disabled:opacity-60">{loading ? 'Memproses...' : <>Kirim Tautan Reset <ArrowRight className="h-5 w-5" /></>}</button></div>
                <div className="mt-6 rounded-2xl border border-yellow-200 bg-yellow-50 p-4 text-[11px] leading-5 text-slate-600"><b className="text-slate-900">Belum terdaftar?</b> Silakan hubungi tim YUPOS untuk mendapatkan akses.</div>
              </form>
            )}

            {currentView === 'purchase' && (
              <div className="flex flex-1 flex-col justify-center overflow-y-auto p-6 sm:p-8">
                <div className="mb-6"><div className="mb-2 text-xs font-black uppercase tracking-[.16em] text-yellow-600">FULL ACCESS LICENSE</div><h2 className="text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">Dapatkan YUPOS untuk bisnis Anda</h2><p className="mt-2 text-sm leading-6 text-slate-500">Nikmati pengalaman POS profesional dengan fitur lengkap untuk membantu operasional bisnis Anda.</p></div>
                ...
                <div className="mt-5 rounded-2xl border border-yellow-200 bg-yellow-50 p-4"><div className="flex items-start gap-3"><Sparkles className="mt-0.5 h-5 w-5 shrink-0 text-yellow-600" /><div><div className="text-xs font-black text-slate-900">Aktivasi akses YUPOS</div><p className="mt-1 text-[11px] leading-5 text-slate-600">Setelah pembelian, akses Anda akan disiapkan sesuai paket yang dipilih. Setelah aktif, Anda dapat masuk dan menggunakan fitur pemulihan kata sandi secara mandiri.</p></div></div></div>
                <a href={WA_LINK} target="_blank" rel="noopener noreferrer" className="mt-5 flex items-center justify-center gap-2 rounded-2xl bg-blue-600 px-5 py-4 text-sm font-black text-white shadow-lg shadow-blue-600/25 transition hover:bg-blue-700"><MessageCircle className="h-5 w-5" /> Hubungi Tim YUPOS</a>
            ...
            <div className="border-t border-slate-100 px-6 py-4 text-center text-[10px] font-semibold text-slate-400 sm:px-8">YUPOS • One Pos For Everything • Akses khusus pelanggan</div>
          </section>
        </div>
      </div>
    </div>
