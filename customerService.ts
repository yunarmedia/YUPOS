...
export const initialCustomers: Customer[] = [
  {
    id: 'CUST-001', name: 'Budi Santoso', phone: '081234567890', customerCode: 'BU7890',
    visitCount: 8, isMember: true, memberSince: '2024-01-15', totalSpent: 420000,
    lastVisit: new Date(Date.now() - 86400000 * 2).toISOString().split('T')[0],
    notes: 'Pelanggan loyal potong rambut + pomade', createdAt: Date.now() - 86400000 * 90,
  },
  {
    id: 'CUST-002', name: 'Rian Pratama', phone: '085798765432', customerCode: 'RI5432',
    visitCount: 3, isMember: false, totalSpent: 105000,
    lastVisit: new Date(Date.now() - 86400000 * 6).toISOString().split('T')[0],
    notes: 'Favorit capster Rian', createdAt: Date.now() - 86400000 * 30,
  },
  {
    id: 'CUST-003', name: 'Siti Rahmawati', phone: '087811223344', customerCode: 'SI3344',
    visitCount: 12, isMember: true, memberSince: '2023-11-20', totalSpent: 780000,
...
export function saveCustomers(merchantId: string, customers: Customer[]): void {
  try {
    localStorage.setItem(getCustomerStorageKey(merchantId), JSON.stringify(customers));
  } catch (err) {
